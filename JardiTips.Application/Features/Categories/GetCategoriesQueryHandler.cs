...
namespace JardiTips.Application.Features.Categories
{
    ...
    public class GetCategoriesQueryHandler(IUnitOfWork unitOfWork) : IQueryHandler<GetCategoriesQuery, List<CategoryDto>>
    {
        public async Task<List<CategoryDto>> HandleAsync(GetCategoriesQuery query, CancellationToken ct = default)
        {
